...
export function Inventory({ route, navigation }) {
  ...
  const { sessionStatus, sessionDate } = useSelector(({ session }) => session);
  const { scannedData, sredstvo, prevPosition, itemsRemain } = useSelector(
    ({ scan }) => scan
  );
  ...
  const getPrevScan = async () => {
    ...
    setScannedData(data.join("\n"));
  };

  const getSredstvo = async () => {
    let data = await AsyncStorage.getItem("sredstvo");
    if (data[0] === '"') {
      data = data.substr(1).slice(0, -1);
    }
    setSredstvo(data);
  ...
  useEffect(() => {
    ...
    getSessionStatus().then((arr) => {
      ...
      arr[1] != null
        ? setSessionDate(`Сессия была открыта: ${arr[1]}`)
  ...
  const analyze = async (data) => {
    // анализирование, если такой предмет есть в инвентаризаци
            ...
            db.transaction((tx) => {
              tx.executeSql(
                ...
                (_, result) => {
                  ...
                  setItemsRemain(
                    `${scanned}/${left + scanned}, строка: ${pos} `
                  );
                },
                (_, error) => console.log(error)
              );
    ...
    <View style={styles.container}>
      <ScrollView>
        ...
        <View style={styles.sessionInfo}>
          <Text style={sessionStatus ? styles.active : styles.danger}>
            {SESSIONS_INFO[sessionStatus].info}
          </Text>
          <Button
            title={SESSIONS_INFO[sessionStatus].button}
            onPress={() => {
              sessionStatus
                ? setSessionModalVisible(true)
                : (downloadDB(), onSessionChangeHandler(sessionStatus, today));
            }}
          />
          {!sessionStatus && (
            <Icon
          ...
          <View style={styles.prevScan}>
            {scannedData !== null ? (
              ...
              <Text style={{ padding: 20 }}>
                Предыдущих сканирований не было
              </Text>
            )}
          </View>
        </View>
